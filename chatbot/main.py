...
async def getResponse(query: QueryResponseElement, accept_language: str=Header(None)) -> QueryResponseElement:
    logger.debug(f"Recieved query: {query}\nheader: {accept_language}")
    if query.is_rag_used is None or query.is_rag_used not in [True, False]:
        ...
        logger.debug(f" ***** Query object ***** {query.model_dump_json()}")
        await retrieveResponse(query=query, language=language)
        ...
        logger.debug(f" ***** Response object ***** {query.model_dump_json()}")
        return query
